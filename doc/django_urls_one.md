深入理解 django 的 urls 的实现 （一）
===================================

改写一个简单 wsgi 服务器
------------------------

  为了摸清楚 django 的 urls 实现，我需要从最原始的状态开始实现它，那么这里的原始状态指的是，首先有一个实现了 wsgi 协议的服务器，
以及满足 wsgi 协议的请求响应。

  由于 django 默认实现了一个满足 wsgi 协议的服务器， 那么我稍作修改就可以拿过来用了， 我尽可能的简化这个服务器，但让它足够清晰。
下面是我修改 django 服务器之后的代码, 我将其保存到 run_server.py 文件中:
...
定义满足 wsgi 的 application 函数
--------------------------------

按照上面所说， 那么接下来我就要实现这么一个 application ， 以方便我测试服务器以及进行后期的升级改造

首先新建一个 python 文件， 叫它 my_wsgi.py , 在文件中添加一个 application 函数：
...
根据 wsgi 协议， 知道 path 是存放在 environ 的 PATH_INFO 当中， 它是要给字符串类型, 修改 application 函数如下:
...
```
    ...
    def path_to_response(environ):
        ...
        if path == '/':
            ...
            data = '<div>get path2 information</div>'
        else:
            path_not_found = True
        if path_not_found:
...
变得不方便， 比如一个路径 /path/id , 这个id 从 1 到 100 都是合法的，那是不是要写个函数，将它逐一添加到这个路径字典中了，
如果 1 到 100 的路径是其他一样的数据了， 显然这样做冗余太大了， 并且针对于不同的路径可能要编写不同的函数来将这些路径添加到
